...
def shape_compare(file_name_list, var_name_list, extra_name="shape"):
	...
	for file_name in file_name_list:
		...
		for var_name in var_name_list:
			...
			if 'pt' in var_name:
				s_dic[var_name].set_frame(xtitle="pt[GeV]",ytitle="a.u.")	
			elif 'mass' in var_name:
				s_dic[var_name].set_frame(xtitle="mass[GeV]",ytitle="a.u.")
			elif 'eta' in var_name:
				s_dic[var_name].set_frame(xtitle="eta",ytitle="a.u.")
	for var_name in var_name_list:
		s_dic[var_name].draw()
...
def stack_hist(file_name_list, var_name_list, extra_name="stack", bkg=True):
	...
	for file_name in file_name_list:
		...
		for var_name in var_name_list:
			...
			if 'pt' in var_name:
                                s_dic[var_name].set_frame1(ytitle="Event")
				s_dic[var_name].set_frame2(xtitle="pt[GeV]")
                        elif 'mass' in var_name:
				s_dic[var_name].set_frame1(ytitle="Event")
				s_dic[var_name].set_frame2(xtitle="mass[GeV]")
                        elif 'eta' in var_name:
				s_dic[var_name].set_frame1(ytitle="Event")
				s_dic[var_name].set_frame2(xtitle="eta")
	for var_name in var_name_list:
		s_dic[var_name].draw()
		s_dic[var_name].Print("./PDFs/"+var_name+"_"+extra_name+".png")
	input_file.Close()
...
if __name__=="__main__":
	...
	stack_hist(file_name_list,var_name_list)
	stack_hist(file_name_list,var_name_list,"stack_nobkg",False)
